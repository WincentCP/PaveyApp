...
class ExpenseCreate(BaseModel):
    trip_id: str
    amount: int          # dalam IDR/Rp
    category: str        # "food", "transport", "activity", dll
    description: str

@router.post("/expenses")
async def add_expense(data: ExpenseCreate, authorization: str = Header(...)):
...
async def get_expenses(trip_id: str, authorization: str = Header(...)):
    token = authorization.replace("Bearer ", "")
    user = supabase.auth.get_user(token)
    if not user:
        raise HTTPException(status_code=401, detail="Unauthorized")
