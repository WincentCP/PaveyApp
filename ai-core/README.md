...
| **Contextual RAG** | Pencarian destinasi berbasis *Cosine Similarity* pada klaster data lokal (atraksi & kuliner) yang disintesis dan divalidasi langsung terhadap data koordinat serta *place details*. Mendukung penyaringan dinamis menggunakan parameter `exclude_names` untuk meniadakan tempat-tempat yang sudah dihasilkan pada hari sebelumnya. |
...
| **API Caching & Storage** | **Redis Cache Layer** terpasang pada gerbang pemanggilan API eksternal untuk menyimpan data cuaca dan rute terdekat. Menggunakan format kunci **Cache Key v7** yang mengintegrasikan list `exclude_names` terurut untuk mencegah tabrakan cache selama itinerary multi-hari. |
