...
# Import metrik dan manager telemetri dari monitoring suite kamu
...
app = FastAPI(
    ...
    version="2.0.0",
    description="Sistem API Rekomendasi RAG Pariwisata berbasis Intelijen Cuaca, Spasial OSRM, dan MLOps Suite Telemetri."
)
...
# Skema Data Validation (Pydantic) - duration_per_place diubah menjadi List[int]
...
def generate_itinerary(payload: TravelPlannerRequest):
    ...
    # [AIOps Monitoring] Catat panjang preferensi untuk deteksi Data Drift via Gauge lama/baru
    if 'INPUT_LENGTH_GAUGE' in globals() or 'INPUT_LENGTH_GAUGE' in locals() or 'TelemetryManager' in globals():
        try:
            INPUT_LENGTH_GAUGE.set(len(payload.preference))
        except Exception:
            pass
    ...
    for i in range(payload.num_places):
        # Jika array yang dikirim FE lebih pendek dari num_places, pakai fallback 60 menit
        if i < len(raw_durations):
            dur = raw_durations[i]
            # Validasi aturan bisnis: kalau durasi <= 0 atau > 8 jam (480 menit), normalkan ke 60 menit
            if dur is None or dur <= 0 or dur > 480:
                validated_durations.append(60)
            else:
                validated_durations.append(dur)
        else:
            # Fallback jika elemen array kurang
            validated_durations.append(60)

    # ----------------------------------------------------------------------
    # LAYER 1: API CACHING STRATEGY (Redis) - Gunakan tuple string dari array durasi
    # ----------------------------------------------------------------------
    ...
    # Ambil kandidat rekomendasi lebih banyak (x4) agar aman pas proses filtering
    ...
    # 2. Filter Tingkat Harga (Price Level) dengan toleransi Free Places
    if req_price_level is not None:
        if "price_level" in candidate_df.columns:
            if req_price_level == 0:
                filtered_df = candidate_df[(candidate_df["price_level"] == 0) | (candidate_df["price_level"].isna())]
    ...
    # ----------------------------------------------------------------------
    is_raining = weather_service.check_is_raining(payload.city) if hasattr(weather_service, 'check_is_raining') else weather_service.is_raining_at_destination(payload.city)
    ...
    # Potong final dataframe sesuai jumlah tempat yang diminta user
    final_rec_df = candidate_df.head(payload.num_places)

    # ----------------------------------------------------------------------
    # LAYER 4: GEOGRAPHICAL ROUTING INJECTION (OSRM)
    # ----------------------------------------------------------------------
    places_records = final_rec_df.to_dict(orient="records")
    rag_context_builder = ""

    # Inisialisasi waktu berjalan awal berdasarkan input user
    ...
    for index in range(len(places_records)):
        ...
        # Ambil durasi spesifik untuk tempat indeks ini dari array yang sudah divalidasi
        current_place_duration = validated_durations[index]

        # 1. Ambil estimasi waktu perjalanan ke titik berikutnya dari OSRM
        if index < len(places_records) - 1:
            next_node = places_records[index + 1]
            func_routing = routing_service.get_travel_duration_minutes if hasattr(routing_service, 'get_travel_duration_minutes') else routing_service.calculate_travel_time
            raw_transit_time = func_routing(
                float(current_node.get("latitude", 0)), float(current_node.get("longitude", 0)),
                float(next_node.get("latitude", 0)), float(next_node.get("longitude", 0))
            )

        # 2. SEKAT UTAMA: HUMAN TIME ROUNDING UP (Pembulatan ke atas per 5 menit)
        ...
        # 3. LINEAR TIME ACCUMULATION LAW
        arrival_string = current_itinerary_time.strftime("%H:%M")

        # Simpan metrik kalkulasi ke dalam node record
        ...
        # RAG Context String Builder - Menyuntikkan durasi kustom dinamis per lokasi
        rag_context_builder += (
        ...
        # Update running datetime untuk destinasi berikutnya: Tiba + Kustom Stay + Transit
    ...
    # Hitung jumlah input token menggunakan Tiktoken backend suite
    input_tokens_count = TelemetryManager.count_tokens(system_instruction + user_generation_prompt)
    ...
    try:
        ...
        output_tokens_count = TelemetryManager.count_tokens(response_payload_text)

        # [AIOps MLOps Suite] Kirim data token telemetri ke Prometheus via TelemetryManager terpusat
        try:
            TelemetryManager.log_inference_telemetry(input_tokens_count, output_tokens_count, "llama-3.1-8b-instant")
        except Exception as prom_err:
            logger.error(f"[Prometheus Error] Gagal push via TelemetryManager: {str(prom_err)}")
