...
export default function PlaceList({ places, showIndex = false }: { places: ChatPlace[]; showIndex?: boolean }) {
    ...
    return (
        ...
        {places.map((p, i) => (
            ...
            <span className={`self-start text-[10px] font-semibold px-2 py-0.5 rounded-full border ${TYPE_COLOR[p.type] ?? TYPE_COLOR.destination} capitalize`}>
            {p.category ?? p.type}
            </span>
            {p.description && (
