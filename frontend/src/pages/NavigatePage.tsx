...
export default function NavigatePage() {
  ...
  const distRemain = useMemo(() => {
    const val = current.distanceKm * (1 - progress);
    return val > 0.01 ? `${val.toFixed(2)} km` : '< 0.1 km';
  }, [progress, current]);
        ...
        <AnimatePresence>
          {altPlace && (
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <img src={altPlace.image} alt="" referrerPolicy="no-referrer" className="w-10 h-10 rounded-xl object-cover shrink-0" />
                <div className="flex-1 min-w-0">
                  <div className="font-semibold text-ink-900 text-sm truncate">{altPlace.name}</div>
                  <div className="text-xs text-ink-500">{altPlace.distanceKm > 0.01 ? `${altPlace.distanceKm.toFixed(2)} km` : '< 0.1 km'} away · {altPlace.durationMin}min</div>
                </div>
            ...
            <motion.div key="active" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <div className="flex items-center gap-3 mb-3">
                <div className="relative shrink-0">
                  <img src={current.image} alt="" referrerPolicy="no-referrer" className="w-14 h-14 rounded-xl object-cover" />
              ...
              {next && (
                <div className="mt-2.5 flex items-center gap-2 text-[11px] text-ink-500 bg-ink-50 rounded-xl px-3 py-2">
                  ...
                  <img src={next.image} alt="" referrerPolicy="no-referrer" className="w-5 h-5 rounded-lg object-cover" />
